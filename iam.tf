    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${element(split("oidc-provider/", "${aws_iam_openid_connect_provider.oidc_provider.arn}"), 1)}:sub": "system:serviceaccount:kube-system:cluster-autoscaler"
          }
        }        
      },
    ]
...
#resource "aws_iam_role_policy_attachment" "eks-AmazonEC2SSM" {
